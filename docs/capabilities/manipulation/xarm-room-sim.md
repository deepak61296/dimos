...
From `dimos shell`, scan with the room prompt set exported by
`dimos.robot.manipulators.xarm.blueprints.simulation.XARM_ROOM_PROMPTS`, then
call `ManipulationModule.refresh_obstacles()` and
`ManipulationModule.get_obstacles()` to inspect what entered the planning
world. The prompt set includes a `green ring` fallback because the tape loses
